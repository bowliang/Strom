...
namespace strom
{
    ...
    void TreeManip::extractNodeNumberFromName(Node *nd, std::set<unsigned> &used)
    {
        assert(nd);
        bool success = true;
        unsigned x = 0;
    ...
    void TreeManip::extractEdgeLen(Node *nd, std::string edge_length_string)
    {
        assert(nd);
        bool success = true;
        double d = 0.0;
    ...
    unsigned TreeManip::countNewickLeaves(std::string newick)
    {
        boost::regex taxonexpr{"[(,]\\s*(\\d+|\\S+?|['].+?['])\\s*(?=[,):])"};
        boost::regex_token_iterator<std::string::iterator> m1{newick.begin(), newick.end(), taxonexpr};
        boost::regex_token_iterator<std::string::iterator> m2;
        return (unsigned)std::distance(m1, m2);
    }

    void TreeManip::stripOutNexusComments(std::string &newick)
    {
        boost::regex commentexpr{"\\[.*?\\]"};
        newick = boost::regex_replace(newick, commentexpr, std::string(""));
    }

    Node *TreeManip::findNextPreorder(Node *nd)
    {
        assert(nd);
        Node *next = 0;
    ...
    bool TreeManip::canHaveSibling(Node *nd, bool rooted, bool allow_polytomies)
    {
        assert(nd);
        if (!nd->_parent)
        {
    ...
    void TreeManip::rerootAtNodeNumber(int node_number)
    {
    ...
    void TreeManip::rerootAtNode(Node *prospective_root)
    {
